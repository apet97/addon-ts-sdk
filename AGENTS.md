...
| Command                                    | Checks                                                                                                                                                                                                                |
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `npm run ci:verify`                        | Canonical root gate: workspace dependency tree, type-check, generated drift, tests, lint, format check, build, public API snapshot, dist import smoke, pack dry-run, installed-consumer smoke, and audits.            |
...
| `npm run verify:package-consumer`          | Packs the already-built package with scripts ignored, installs it into temporary ESM/CJS/TypeScript consumers, imports public subpaths, signs/verifies test tokens, type-checks declarations, and serves `/manifest`. |
