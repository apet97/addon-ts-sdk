...
const expectFn = (label, v) => { if (typeof v !== "function") fail(`${label} is not a function (got ${typeof v})`); };
const expectObj = (label, v) => { if (v === null || typeof v !== "object") fail(`${label} is not an object (got ${typeof v})`); };

// --- 1. The emitted ESM entry must expose the public API as real named exports. ---
const esm = await import(path.join(dist, "esm", "index.js")).catch((e) => fail("ESM import threw: " + e.message));
const esmAdapters = await import(path.join(dist, "esm", "adapters", "index.js")).catch((e) => fail("ESM /adapters import threw: " + e.message));
for (const name of ["ClockifyAddon", "ClockifyManifest", "ClockifyComponent", "ClockifyScope", "ClockifySignatureParser"]) {
...
if (esm.ClockifyScope.PROJECT_READ !== "PROJECT_READ") fail("esm.ClockifyScope.PROJECT_READ missing");
...
const esmClockify = await import(path.join(dist, "esm", "clockify", "index.js")).catch((e) => fail("ESM /clockify import threw: " + e.message));
const esmTesting = await import(path.join(dist, "esm", "testing", "index.js")).catch((e) => fail("ESM /testing import threw: " + e.message));
...
addon.registerComponent(
  ClockifyComponent.v1_4Builder().activityTab().allowAdmins().path("/component").label("Activity Tab").build(),
  async () => ({ status: 200, headers: { "content-type": "text/html" }, body: "<html><body>Hello Clockify!</body></html>" }),
);
...
try {
  ...
  if (res.headers.get("content-type") !== "application/json") fail("GET /manifest content-type not application/json");
