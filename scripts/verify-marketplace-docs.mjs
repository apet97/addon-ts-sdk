...
const provenance = JSON.parse(
  await readFile(new URL("MARKETPLACE_DOCS/provenance.json", root), "utf8"),
);
for (const [name, expected] of Object.entries(provenance.documents)) {
  ...
  if (actual !== expected)
    throw new Error(`${name} changed without a reviewed provenance update.`);
...
console.log(
  `Marketplace docs provenance OK: ${Object.keys(provenance.documents).length} documents.`,
);
